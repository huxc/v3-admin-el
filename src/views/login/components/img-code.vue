...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const drawText = (ctx, txt, i) => {
      ...
      const x = (i + 1) * (contentWidth.value / (defaultData.identifyCode.length + 1))
      const y = randomNum(defaultData.fontSizeMax, contentHeight.value - 5)
    ...
    const makeCode = () => {
      ...
      for (let i = 0; i < 4; i++) {
        defaultData.identifyCode += defaultData.identifyCodes[randomNum(0, defaultData
          .identifyCodes.length)]
      }
